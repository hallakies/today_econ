...
2. **콘텐츠 중복 배제 (Zero Semantic Redundancy)**:
   * 슬라이드 타이틀, 서브타이틀, 불릿 포인트, 그리고 에디터 평(Editor's Insight) 간에 핵심 키워드나 수식어구가 중복 노출되는 현상을 철저히 금지한다.
3. **가독성 극대화 (Extreme Text Diet)**:
   * 카드당 줄글 형태를 배제하고, 모바일 화면에서 1초 만에 인지될 수 있도록 불릿 포인트를 공백 포함 22자 내외의 명사형 종결 문구로 정리한다.
4. **쉬운 지식 전달**:
   * 어려운 경제 용어는 괄호를 사용하여 초보자의 눈높이에 맞게 아주 쉽게 비유 또는 보완 설명을 덧붙인다.
5. **실천적 조치 유도 (Actionable Insights)**:
   * 단순히 팩트 전달에 그치지 않고, 카드의 마지막 장에서 독자가 오늘 당장 스마트폰을 켜거나 해당 기관에 직접 조치를 취할 수 있는 구체적인 행동 대책(Action Item)을 유추하여 제시한다.
...
### 디자인 디테일 정책
* **배경 이미지**: 3D abstract fluid (입체적 곡선, 파동 등) 이미지를 사용한다. 실사 인물 사진, 사무실 책상, 클립아트 등 조잡하거나 일관성 없는 이미지는 사용하지 않는다.
...
3. **`src/generator.js`**: 선정된 뉴스를 바탕으로 슬라이드 3개 분량의 타이틀, 설명, 이미지 생성 프롬프트, 그리고 인스타그램 본문 멘트를 만듭니다. 여기서 **`validateAndRepairContent`** 함수를 거쳐 명사형 종결, 조사 제거, 글자 제한(불릿 22자, 인사이틀 38자) 및 중복 배제가 물리적으로 한 번 더 필터링됩니다.
...
4. **절대 문장형 불릿을 쓰지 말 것**: 에디터 평이나 불릿은 반드시 명사구 또는 간결한 행동지침형 단문으로 마쳐야 모바일 피드 가독성을 유지할 수 있습니다.
