...
선택된 경제 뉴스를 바탕으로 인스타그램 릴스(Reels) 영상 ᄆ�    - **카드 3(card3)**: 기사 사건이 독자의 지갑에 미칠 영향과 독자가 취해야 할 실생활 행동 지침(Action) 3가지입니다. (배지명 추천: "그래서 어떻게 돼?")
    - **카피라이팅 톤앤매너 변경 (Actionable CTA)**: 카드 3(card3)의 불릿 포인트와 에디터 평은 절대로 건조한 명사형 요약이나 나열(예: 'IR 자료 바로 확인', '해외 전력 투자 검토')로 작성하지 마십시오. 유저에게 직접 말을 거는 듯한 행동 유도(CTA) 문장(예: "지금 당장 DART에서 IR 자료를 열어보세요!", "내 계좌의 에너지 섹터 비중, 지금이 점검할 타이밍입니다!")으로 역동적인 톤앤매너를 변환하여 작성하십시오.
      각 불릿 포인트는 20자 내외의 구체적인 행동 팁이어야 합니다. 두 카드의 불릿 포인트는 절대 겹치거나 같아서는 안 되며, 완전히 구분되어야 합니다.��ᅡ리 집이면 최대 얼마까지 대출해줄지 정하는 비율)")
...
### 응답 JSON 스키마:
{
  ...
  "card3": {
    ...
    "bullets": [
      "15자 내외 행동 유도 CTA 1 (예: 고금리 예적금 막차, 당장 가입하세요!)",
      "15자 내외 행동 유도 CTA 2 (예: 고정금리 대환 수수료부터 확인해볼 것!)",
      "15자 내외 행동 유도 CTA 3 (예: 미국 장기채 ETF 분할 매수 타이밍입니다)"
    ],
    "editors_insight": "대책에 대한 행동 유도 에디터 평 (예: 대출 갈아타기 타이밍을 당장 모니터링하세요!)",
    "image_prompt": "FLUX 이미지 생성용 영어 프롬프트"
  },
...
위의 기사 내용을 분석하여 카드 뉴스 원고와 이미지 프롬프트, 인스타그램 멘트를 생성해 주세요.`;
  ...
  try {
    ...
    try {
      ...
      if (!resultJson.card1 || !resultJson.card2 || !resultJson.card3 || !resultJson.card3.image_prompt) {
        throw new Error('Missing required schema fields (card1, card2, or card3) in LLM output');
      }
    } catch (apiError) {
      console.warn('[Generator] 70B/120B failed, rate-limited, or generated invalid JSON. Error:', apiError.message || apiError);
