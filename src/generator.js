...
2. **카드 작성 (cards)**:
   ...
   - **card3 (그래서 어떻게 돼?)**: 
     * 첫 번째 불릿: "[나에게 미치는 영향]" 이라는 접두어로 시작하여, 내 지갑과 실생활에 미치는 진짜 영향을 서술형으로 씁니다.
     * 두 번째 불릿: "[오늘의 액션 플랜]" 이라는 접두어로 시작하여, 당장 실천할 수 있는 뾰족한 행동 지침을 "~해보세요." 형태로 씁니다.
...
반드시 마크다운 백틱 없이 순수한 JSON 포맷으로만 응답하세요.
{
  ...
  "cards": {
    ...
    "card3": {
      ...
      "bullets": [ "[나에게 미치는 영향] 상세 설명", "[오늘의 액션 플랜] 구체적 액션 아이템" ],
      "hard_terms": []
    }
  }
}`;
