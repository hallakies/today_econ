...
const CUT_OFF_PARTICLE_ENDING = /\S+(?:을|를|이|가|은|는|의|에|와|과|부터|까지)(?:이에요|입니다)[.!?]?$/u;
...
function evaluateBrandPromise(content, sourceText = '') {
  ...
  const selectedHookType = analysis.selected_hook_type;
  ...
  const titleNumbers = extractMaterialNumbers(title);
  const titleUsesGroundedHeadlineNumber = selectedHookType === 'headline_edit'
    && titleNumbers.length > 0
    && titleNumbers.some(number => normalizeSource(sourceText).includes(number));
  if (
    strongestNumbers.length > 0
    && !strongestNumbers.some(number => normalizedTitle.includes(number))
    && !titleUsesGroundedHeadlineNumber
  ) {
...
function evaluateContentQuality(content, sourceText = '') {
  ...
  if (!/(내|부모|가족|월|상환|이자|대출|집값|주택|청약|세금|생활비|노후|사업|자영업|투자|예금|채권|수익|지출|현금흐름)/.test(`${title} ${subtitle}`)) {
  ...
  for (const [key, expectedTitle] of Object.entries(FRIENDLY_SECTIONS)) {
    ...
    card.bullets.forEach((bullet, index) => {
      ...
      if (CUT_OFF_PARTICLE_ENDING.test(plain)) fail(`${key} bullet ${index + 1} ends with a cut off particle`, 25);
