...
3.  **Llama 뉴스 셀렉터 (`src/selector.js`)**: 지난 7일간 다룬 뉴스 내역(`history.json`)을 기반으로 중복 뉴스(예: 일일 증시 소폭 등락 등)를 걸러내고, 오늘 다룰 최고 가치의 핵심 뉴스 1개를 **Groq Llama 3.3 70B** 모델로 정밀 판별.
4.  **원고 및 프롬프트 생성기 (`src/generator.js`)**: 뉴스 내용을 일반 독자 눈높이에 맞춰 순화하고 어려운 단어를 괄호로 설명. 또한 **Hugging Face FLUX.1-schnell**용 이미지 묘사 프롬프트(영어) 및 이모지가 가득한 인스타그램 본문 멘트를 완벽한 JSON 구조로 생성.
5.  **카드 뉴스 이미지 렌더러 (`src/renderer.js`)**:
    *   Hugging Face API 토큰을 사용해 FLUX.1-schnell 일러스트를 요청하여 고품질 이미지를 획득.
    *   API 제한 또는 크레딧 부족 시 **Pollinations.ai 무료 API**로 자동 Fallback 하도록 설계하여 안정성 강화.
...
*   **스마트 테마 및 강조 색상 동적 매칭**: Llama 모델이 판단한 기사 내용 카테고리(거시금융/실생활/IT빅테크)에 맞춰 적절한 템플릿 테마와 강조 색상(Theme Color)을 실시간 매핑하여 최적의 비주얼을 자율 생성합니다.
